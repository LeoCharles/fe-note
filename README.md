# 前端学习笔记

## HTML

## CSS

## JavaScript

+ [执行上下文与变量对象](JavaScript/01-执行上下文与变量对象.md)
+ [作用域链与闭包](JavaScript/02-作用域链与闭包.md)
+ [this 对象](JavaScript/03-this对象.md)
+ [函数](JavaScript/04-函数.md)
+ [创建对象](JavaScript/05-创建对象.md)
+ [原型](JavaScript/06-原型.md)
+ [继承](JavaScript/07-继承.md)
+ [JS 基础](JavaScript/JS基础.md)

## ECMAScript

+ [ES6 基础](ECMAScript/ES6基础.md)
+ [Promise](ECMAScript/Promise.md)

## Web API

+ [IndexedDB](WebAPI/IndexedDB.md)
+ [WebWorker](WebAPI/WebWorker.md)
+ [WebSocket](WebAPI/WebSocket.md)

## Vue

+ [Vue 学习基础](Vue/Vue基础.md)
+ [Vue 框架原理](Vue/Vue框架原理.md)
+ [Vue 最佳实践](Vue/Vue最佳实践.md)
+ [Vuex](Vue/vuex.md)

## React

+ [React 基础](React/React基础.md)
+ [React Hooks](React/hooks.md)

## TypeScript

+ [TypeScript 基础](TypeScript/TypeScript基础.md)

## 浏览器

+ [Browser](ECMAScript/BOM/Browser.md)
+ [DOM](ECMAScript/DOM/DOM.md)
+ [EventTarget](ECMAScript/DOM/EventTarget.md)

## 微信

+ [微信小程序](WX/微信小程序.md)

## 工具库

+ [工具方法](Tool/utils.js)

## 笔记

+ [前端性能优化](Note/前端性能优化.md)
+ [Git 常用命令](Note/Git常用命令.md)
+ [常用 Linux 基础命令](Note/常用Linux基础命令.md)
+ [lodash 常用方法](Note/lodash常用方法.md)
+ [Go 开发配置](Note/Go开发配置.md)
+ [网络](Note/网络.md)
+ [JWT](Note/JWT.md)
+ [排序](Note/排序.md)
+ [Vue 项目移动页面真机调试](Note/Vue项目移动页面真机调试.md)
+ [移动 Web 开发](Note/移动Web开发.md)
